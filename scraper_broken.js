...
// Translation dictionary for common products
const PRODUCT_TRANSLATIONS = {
  ...
  // Add more languages and products as needed
  // Spanish to Turkish
  'leche': 'süt',
  'pan': 'ekmek',
  'queso': 'peynir',
...
// Function to translate query to Turkish
function translateToTurkish(query) {
  const lowerQuery = query.toLowerCase().trim();
  return PRODUCT_TRANSLATIONS[lowerQuery] || query;
}
...
async function searchMultiple(product) {
  ...
  return {
    marketfiyati: entries.find(([m]) => m === "marketfiyati")?.[1] || [],
    sok: entries.find(([m]) => m === "sok")?.[1] || [],
    metro: entries.find(([m]) => m === "metro")?.[1] || [],
    errors,
  };
}
