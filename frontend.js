...
function updateNavbar() {
  ...
  if (comingSoonPanel) comingSoonPanel.hidden = true;

  if (navCartBtn) navCartBtn.hidden = false;
  if (navOrdersBtn) navOrdersBtn.hidden = false;

  if (isLoggedIn) {
    if (navUser) navUser.textContent = userName || "";
    if (navProfileBtn) navProfileBtn.hidden = false;
    if (navLogoutBtn) {
