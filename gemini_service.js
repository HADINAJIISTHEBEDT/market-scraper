...
async function searchWithGemini(query, market = null) {
  ...
  try {
    ...
    const prompt = market ? 
      `Search for grocery products related to "${query}" specifically from ${market} market in Turkey. 
      Return exactly 7 products with their names, prices in Turkish Lira, and image URLs.
    ...
    const validProducts = products.filter(product => {
      ...
      const productMarket = normalizeText(product.market || market || 'Akakce');
    ...
    return validProducts.map(product => ({
      market: normalizeText(product.market || market || 'Akakce'),
...
async function searchMultipleWithGemini(query) {
  ...
  try {
    ...
    if (generalProducts.length > 0) {
      results.akakce = generalProducts;
    }
  } catch (error) {
